...
DATA = ROOT / 'data'
source = '''Senior Python Developer|Payne, Roberts and Davis|Stewartbury, AA
...
for i,line in enumerate(source.splitlines()):
    title, company, location = line.split('|')
    slug = re.sub(r'[^a-z0-9]+','-',title.lower()).strip('-') + f'-{i}'
    category_hint = title.split()[0]
    desc = (f"{title} role at {company}. The position involves professional responsibilities, teamwork, "
            f"problem solving, communication, reporting and domain-specific tasks. Candidates should be "
            f"organized, responsible and willing to learn. This is a demonstration description for the bundled dataset.")
    rows.append({
        'job_title': title,
        ...
        'apply_link': f'https://realpython.github.io/fake-jobs/jobs/{slug}.html'
    })
